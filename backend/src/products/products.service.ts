...
export class ProductsService {
  ...
  async addShopProduct(shopId: number, data: any) {
    const [shopProduct] = await this.dbService.db.insert(shopProducts).values({
      shopId,
    ...
    const [product] = await this.dbService.db.select().from(products).where(eq(products.id, shopProduct.productId)).limit(1);
    return { ...shopProduct, product };
  }

  async updateShopProduct(shopId: number, id: number, data: any) {
    const [shopProduct] = await this.dbService.db.update(shopProducts).set({
      unitPrice: data.unitPrice,
      isActive: data.isActive,
      updatedAt: new Date(),
    }).where(and(eq(shopProducts.id, id), eq(shopProducts.shopId, shopId))).returning();
  ...
  async deleteShopProduct(shopId: number, id: number) {
    await this.dbService.db.delete(shopProducts).where(and(eq(shopProducts.id, id), eq(shopProducts.shopId, shopId)));
    return { success: true };
  }
  ...
  async lookupShopProductByBarcode(shopId: number, code: string) {
    ...
    if (result.length === 0) throw new NotFoundException('Shop product not found');
    return result[0];
  }
  ...
  async searchGlobalProducts(query?: string) {
    if (!query) {
      return await this.dbService.db.select().from(products).where(eq(products.status, 'approved'));
    }
    return await this.dbService.db.select().from(products)
      .where(and(
  ...
  async lookupGlobalProductByBarcode(code: string) {
    const [product] = await this.dbService.db.select().from(products).where(eq(products.barcode, code)).limit(1);
  ...
  async addGlobalProduct(data: any, user: any) {
    ...
    if (data.barcode) {
      const [existing] = await this.dbService.db.select().from(products).where(eq(products.barcode, data.barcode)).limit(1);
      if (existing && existing.status === 'approved') {
        throw new BadRequestException('A verified product with this barcode already exists. Please use it.');
      }
    }

    const [product] = await this.dbService.db.insert(products).values({
  ...
  async updateGlobalProduct(id: number, data: any, user: any) {
    const [product] = await this.dbService.db.select().from(products).where(eq(products.id, id)).limit(1);
    if (!product) throw new NotFoundException('Product not found');
    ...
    const [updatedProduct] = await this.dbService.db.update(products).set({
      ...data,
      updatedAt: new Date(),
