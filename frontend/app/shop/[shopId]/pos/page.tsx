      ...
      {receiptModalOpen && generatedBill && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-canvas border border-hairline rounded-2xl shadow-level-4 max-w-sm w-full p-6 relative flex flex-col max-h-[90vh]">
            ...
            {/* Thermal Print Receipt Area */}
            <div className="flex-1 overflow-y-auto bg-white border border-hairline-strong/30 rounded-lg p-5 font-mono text-[10px] text-zinc-800 shadow-inner select-text select-all print:border-none print:shadow-none">
              <div className="text-center space-y-1 mb-4">
                ...
                {shop?.gstNumber && <p className="font-bold">GSTIN: {shop.gstNumber}</p>}
              </div>

              <div className="border-t border-dashed border-zinc-400 py-2 space-y-1">
                <div className="flex justify-between">
                  <span>BILL NO:</span>
                  <span className="font-bold text-black">{generatedBill.billNumber}</span>
                </div>
                <div className="flex justify-between">
                  <span>DATE:</span>
                  <span>{new Date(generatedBill.createdAt).toLocaleString("en-IN")}</span>
                </div>
                <div className="flex justify-between">
                  <span>CASHIER:</span>
                  <span>{generatedBill.cashierName || "Standard Cashier"}</span>
                </div>
              </div>
              ...
              <div className="py-2.5 space-y-1.5 border-b border-dashed border-zinc-400 text-black">
                ...
                <div className="flex justify-between text-xs font-extrabold border-t border-dotted border-zinc-400 pt-1.5">
                  <span>GRAND TOTAL:</span>
                  <span>₹{(grandTotalPaise / 100).toFixed(2)}</span>
                </div>
              </div>

              {/* Receipt Footer */}
              <div className="text-center pt-4 space-y-1 text-zinc-600">
            ...
            {/* Actions */}
            <div className="mt-4 flex gap-3.5 pt-3 border-t border-hairline shrink-0">
              <button
                onClick={() => {
