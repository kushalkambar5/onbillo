...
export interface OptionWheelProps {
  ...
  /** Controlled selected index. When provided, the wheel animates to it. */
  selected?: number;
...
export default function OptionWheel({
  ...
  // Lazily read the root font size once per mount (SSR-safe: falls back to 16).
  const [remPx] = useState(() => getRemPx());

  const cfg = useMemo<WheelConfig>(
    () => ({
      count: items.length,
      items,
    ...
    [items, fontSize, spacing, remPx, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, draggable, soundUrl, soundVolume]
  );
  ...
  // Stable rAF entry point; the real layout step is assigned in the effect
  // below so the callback never references itself during render.
  const frameFnRef = useRef<(now: number) => void>(() => {});
  const runFrame = useCallback((now: number) => {
    frameFnRef.current(now);
  }, []);
  ...
  useEffect(() => {
    ...
    frameFnRef.current = (now: number) => {
      ...
      const R = tiltRad > 0.0005 ? config.rowH / tiltRad : 0;
      for (let i = 0; i < n; i++) {
        const el = els[i];
        if (!el) continue;
        let d = i - next;
        ...
        if (R > 0) {
          ...
          y = R * Math.sin(ang);
          x = -mirror * R * (1 - Math.cos(ang)) * config.curve;
          rot = (mirror * ang * 180) / Math.PI;
        }
        el.style.transform = `translate(${x.toFixed(2)}px, calc(${y.toFixed(2)}px - 50%)) rotate(${rot.toFixed(3)}deg)`;
        el.style.opacity = String(Math.max(config.minOpacity, 1 - dist * config.fade));
  ...
  // Controlled mode: animate the wheel when the parent drives `selected`.
  useEffect(() => {
    if (selected == null || cfgRef.current.count === 0) return;
    const clamped = Math.min(Math.max(Math.round(selected), 0), cfgRef.current.count - 1);
  ...
  return (
    <div
      ...
      aria-activedescendant={`option-wheel-item-${selectedIndex}`}
