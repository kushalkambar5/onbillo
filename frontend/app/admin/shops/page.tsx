    ...
    <Skeleton name="admin-shops" loading={loading}>
      <div className="space-y-8 select-none">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 border-b border-hairline/80 pb-6">
          <div>
            <h1 className="text-xl font-bold text-foreground font-sans">Shop Workspace Directory</h1>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            {/* Time range switcher */}
            <div className="flex items-center gap-1 bg-canvas/60 p-1 border border-hairline/80 rounded-xl">
        ...
        <div className="bg-canvas border border-hairline/80 rounded-2xl overflow-hidden shadow-md">
          {filteredShops.length === 0 ? (
            <div className="p-12 text-center text-mute">
              <Store className="w-10 h-10 mx-auto text-mute mb-3" />
              <h4 className="text-xs font-bold text-foreground">No shops found</h4>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-hairline text-xs">
                  {filteredShops.map((s) => (
                    <tr key={s.id} className="hover:bg-canvas-soft/40 transition-colors">
                      ...
                      <td className="py-3.5 px-5 text-body font-mono">
                        <span className="flex items-center gap-1.5">
                          <Phone className="w-3.5 h-3.5 text-mute shrink-0" />
                          {s.phone || "—"}
                        </span>
                      </td>
                      <td className="py-3.5 px-5 text-body font-medium max-w-[200px] truncate" title={`${s.addressLine1}, ${s.city}, ${s.state}`}>
                        <span className="flex items-center gap-1">
                          <MapPin className="w-3 h-3 text-mute shrink-0" />
                          {s.city}, {s.state}
                        </span>
                      </td>
                      <td className="py-3.5 px-5 text-center font-bold text-foreground font-mono">
                        <span className="inline-flex items-center gap-1 bg-canvas-soft px-2 py-0.5 rounded border border-hairline text-body">
                      ...
                      <td className="py-3.5 px-5 font-bold text-foreground font-mono">
                      ...
                      <td className="py-3.5 px-5 text-center">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-canvas-soft border border-hairline text-body capitalize">
                          {s.taxType.replace("_", " ")}
                        </span>
                      </td>
                      <td className="py-3.5 px-5 text-center text-mute font-mono">
                        <span className="flex items-center justify-center gap-1">
                          <Calendar className="w-3 h-3 text-mute shrink-0" />
